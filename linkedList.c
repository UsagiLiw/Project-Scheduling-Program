 ...
 *
 *  Created by Sally Goldin, 30 November 2011 for CPE113
 *  Modified 7 January 2013 to simplify (single list)
 *  Modified 17 January 2014 - remove now checks for
 *                       single item list before general case
 */
...
void* listRemoveEnd()
{
   ...
   if (linkedList != NULL)
      ...
      if (linkedList->tail != NULL)	  
         {
	 LISTITEM_T* pTail  = linkedList->tail; /* this is the one we
                                                 * will remove.
                                                 */
	 LISTITEM_T* pThisItem = linkedList->head;
         ...
         if (pThisItem == pTail)
	    {
	    linkedList->head = NULL;  /* when we remove the tail the list will
				       * be empty.
				       */

	...
	*pPredecessor = NULL;
	...
	else if (count > 1)
	{
		...
		while (pCurrentItem != NULL)
		{
			...
			*/
			if ((pMinItem == NULL) || ((*compare)(pCurrentItem->data,pMinItem->data) < 0))
			{
...
/* Use the comparison function to find the biggest
 * item currently in the list
 * Argument
 *  pPredecessor  -  Used to return the previous item
 *                   so we can unlink the minimum
 ...
 * if the queue is empty.
 */
LISTITEM_T * findMaximum(LISTITEM_T ** pPredecessor)
{
	LISTITEM_T * pMaxItem = NULL;
	LISTITEM_T * pCurrentItem = NULL;
	LISTITEM_T * pPredItem = NULL;
	*pPredecessor = NULL;
	if (count == 1)
	...
	else if (count > 1)
	{
		...
		while (pCurrentItem != NULL)
		{
			...
			* this item is less than the current min
			*/
			if ((pMaxItem == NULL) || ((*compare)(pCurrentItem->data,pMaxItem->data) > 0))
			{
				pMaxItem = pCurrentItem;
				*pPredecessor = pPredItem;
				/* reset current min and also set predecessor */
			}
			pPredItem = pCurrentItem;
...
void * dequeueMax()
{
	...
	{
		LISTITEM_T* pPredItem = NULL;  /* preceding item */
		LISTITEM_T* pMaxItem = findMaximum(&pPredItem);
		returnData = pMaxItem->data;
		if (pPredItem != NULL)
		{
			pPredItem->next = pMaxItem->next;
		}
		if (pMaxItem == head)
		{
			head = pMaxItem->next;

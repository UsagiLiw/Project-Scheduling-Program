...
int printCriticalPath()
{
	...
	VERTEX_T * pDummy = NULL;
	VERTEX_T * pMax = NULL;
	addVertex(ENDVERTEX,NULL);
	VERTEX_T * pEnd = findVertexByKey(ENDVERTEX,&pDummy);
	...
	while(queueMinSize() > 0)
	{
		pMax = (VERTEX_T*) dequeueMax();
		pMax->color = BLACK;
		ADJACENT_T * pAdjacentEdge = pMax->adjacentHead;
		while(pAdjacentEdge != NULL)
		{
			VERTEX_T * pAdj = pAdjacentEdge->pVertex;
			int distance = pMax->dValue + pAdjacentEdge->weight;
			if((pAdj->color == WHITE) && (distance > pAdj->dValue))
			{
				pAdj->dValue = distance;
				pAdj->parent = pMax;

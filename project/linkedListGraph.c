 ...
 *  Edited By Bhimapaka Thapanangkun
 *	ID: 60070503447
 */
 ...
 * error or running out of space, depToVtxing on how the 
...
/* Print the path From vertex to the destined vertex
 * Arguments
 *		pToVtx - The destination vertex that the user wanted to go*/
void printPath(VERTEX_T* pToVtx)
{
	VERTEX_T** path = calloc(countVertex,sizeof(VERTEX_T*));
	int pathCount = 0;
	int i = 0;
	if (path == NULL) 
	{
	...
	{
		VERTEX_T * pCurrent = pToVtx;
		while (pCurrent != NULL)  
		{
			path[pathCount] = pCurrent;
			pathCount++;
			pCurrent = pCurrent->fromVertex;
		}
		/*Traverse and print through the path that reach the destination*/
...
int isReachable(char* key1, char* key2)
{
	...
	else
	{
		...
		if (pToVtx->color != BLACK)
		{ 
			bOk = 0; 
		}	 
		else
		{
			printf("Path from %s to %s:\n",pFromVtx->key,pToVtx->key); 
			printPath(pToVtx);
			bOk = 1;
		}
	}
...
int updateTaskStatus(char * taskString)
{
	...
	if(pVTX->data->Status == 1)
	{
		while(1)
		{
			...
			printf("Do you want to undo this task[Y/N]? : ");
